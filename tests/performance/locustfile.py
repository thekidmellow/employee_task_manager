    ...
    def login(self):
        ...
        if response.status_code == 200:
            ...
            if login_response.status_code == 302:
                ...
                login_response.failure(f"Manager login failed: {login_response.status_code}")
    ...
    def view_manager_dashboard(self):
        with self.client.get("/dashboard/manager/", catch_response=True) as response:
            if response.status_code == 200:
                ...
                response.failure(f"Manager dashboard failed: {response.status_code}")
    ...
    def view_all_tasks(self):
        with self.client.get("/tasks/", catch_response=True) as response:
            if response.status_code == 200:
                ...
                response.failure(f"All tasks view failed: {response.status_code}")
    ...
    def create_task_form(self):
        with self.client.get("/tasks/create/", catch_response=True) as response:
            if response.status_code == 200:
                ...
                response.failure(f"Create task form failed: {response.status_code}")
    ...
    def api_user_list(self):
        ...
        with self.client.get("/accounts/api/users/", headers=headers, catch_response=True) as response:
            if response.status_code == 200:
                ...
                response.failure(f"User list API failed: {response.status_code}")
    ...
    def test_protected_page(self):
        with self.client.get("/tasks/", catch_response=True) as response:
            if response.status_code == 302:
                ...
                response.failure(f"Protected page access failed: {response.status_code}")
...
class SpikeTestUser(HttpUser):
    ...
    def concurrent_task_access(self):
        self.client.get("/tasks/")
        self.client.get("/dashboard/employee/")
        self.client.get("/tasks/api/stats/", headers={"X-Requested-With": "XMLHttpRequest"})
