...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user:
                login(request, user)
                messages.success(request, f'Welcome to Employee Task Manager, {user.get_full_name() or user.username}!')
                return redirect('core:dashboard')
    else:
...
def profile_view(request):
    ...
    user_stats['completion_rate'] = round(
        (user_stats['completed_tasks'] / user_stats['total_tasks']) * 100, 1
    ) if user_stats['total_tasks'] else 0
...
def delete_account_view(request):
    ...
    if request.method == 'POST':
        ...
        if pending_tasks > 0:
            messages.error(
                request,
                f'Cannot delete account. You have {pending_tasks} pending tasks. '
