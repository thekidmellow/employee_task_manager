...
def task_list_view(request):
    ...
    stats = {
        ...
        "in_progress": stats_queryset.filter(
            status="in_progress"
        ).count(),
...
def task_create_view(request):
    ...
    if request.method == "POST":
        form = TaskCreationForm(request.POST, user=user, auto_id="id_%s")
        if form.is_valid():
        ...
        form = TaskCreationForm(user=user, auto_id="id_%s")
...
def task_update_view(request, task_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f'Task "{updated_task.title}" {action} successfully!',
            )
            return redirect(
                "tasks:task_detail",
...
def task_stats_api(request):
    ...
    qs = (
        Task.objects.all()
        if is_manager
        else Task.objects.filter(assigned_to=user)
    )
